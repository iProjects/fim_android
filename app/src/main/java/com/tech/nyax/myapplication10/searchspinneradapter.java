...
public class searchspinneradapter implements SpinnerAdapter {

    private LayoutInflater inflater;
    List<searchspinnerdto> _lstdata;
    private static final String TAG = searchspinneradapter.class.getSimpleName();
    Context _context;

    public searchspinneradapter(Context context, List<searchspinnerdto> datalst) {
//        super(context, 0, data);
    ...
    public View getDropDownView(int position, View view, ViewGroup parent) {
        ...
        final searchspinnerdto data = getItem(position);
        viewHolder.txtstatus_value.setText(data.getdto_value());
    ...
    @Override
    public searchspinnerdto getItem(int position) {
        return _lstdata.get(position);    // single item in the list
    }

    @Override
    public long getItemId(int position) {
        searchspinnerdto data = getItem(position);
