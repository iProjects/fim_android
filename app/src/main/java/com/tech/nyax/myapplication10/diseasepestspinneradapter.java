...
public class diseasepestspinneradapter implements SpinnerAdapter {

    private LayoutInflater inflater;
    List<diseasepestdto> _lstdata;
    private static final String TAG = diseasepestspinneradapter.class.getSimpleName();
    Context _context;

    public diseasepestspinneradapter(Context context, List<diseasepestdto> datalst) {
//        super(context, 0, data);
    ...
    public View getDropDownView(int position, View view, ViewGroup parent) {
        ...
        final diseasepestdto data = getItem(position);
    ...
    @Override
    public diseasepestdto getItem(int position) {
        return _lstdata.get(position);    // single item in the list
    }

    @Override
    public long getItemId(int position) {
        diseasepestdto data = getItem(position);
